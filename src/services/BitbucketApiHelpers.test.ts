...
describe('bitbucketFetchJson', () => {
  it('fetches and parses JSON data on 200 response', async () => {
    ...
    const result = await Effect.runPromise(
      bitbucketFetchJson<{ uuid: string; slug: string }>(
  ...
  it('passes custom options through to fetch', async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(createMockResponse({ uuid: '{123}' }))
...
describe('bitbucketFetchAllPages', () => {
  it('fetches a single page of data when no next URL', async () => {
    ...
    const result = await Effect.runPromise(
      bitbucketFetchAllPages<{ uuid: string }>(
  ...
  it('uses the exact next URL provided by Bitbucket for subsequent pages', async () => {
    const nextUrl =
      'https://api.bitbucket.org/2.0/repositories/owner/repo/pullrequests?page=2&pagelen=50'
    const page1 = {
      values: [{ uuid: '{1}' }],
      page: 1,
      next: nextUrl,
  ...
  it('includes custom params in the first page URL', async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(createMockResponse({ values: [], page: 1 }))
  ...
  it('handles response with missing values field gracefully', async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(createMockResponse({ page: 1, pagelen: 100 }))
...
describe('buildBitbucketUrl', () => {
  ...
  it('handles self-hosted Bitbucket Server URLs', () => {
    ...
    expect(url).toBe(
      'https://bitbucket.mycompany.com/rest/api/1.0/projects/PROJ/repos',
    )
  })
})
...
describe('bitbucketFetch mutation methods', () => {
  ...
  it('handles PUT requests', async () => {
    ...
    await Effect.runPromise(
      bitbucketFetchJson(
        '/repositories/owner/repo/pullrequests/1',
        BASE_URL,
        TOKEN,
        {
          method: 'PUT',
  ...
  it('handles DELETE requests', async () => {
    ...
    const response = await Effect.runPromise(
      bitbucketFetch(
        '/repositories/owner/repo/pullrequests/1/comments/1',
        BASE_URL,
        TOKEN,
        {
          method: 'DELETE',
